...
async function makeToken(env: Env, userId: number) {
...
const auth = async (c: any, next: any) => {
  ...
  try {
    ...
    if (!user) return err(c, 401, '用户不存在')
    c.set('userId', id)
...
app.post('/auth/register', async (c) => {
  ...
  return c.json({ token: await makeToken(c.env, userId), email })
})
...
app.post('/auth/login', async (c) => {
  ...
  return c.json({ token: await makeToken(c.env, user.id), email })
})
